...
SET
  v_Order = UPPER(p_SortOrder);

END IF;

IF v_Order NOT IN ('ASC', 'DESC') THEN SIGNAL SQLSTATE '45000'
SET
  MESSAGE_TEXT = 'SortOrder debe ser ASC o DESC';
...
IF p_CategoryIds IS NOT NULL
AND TRIM(p_CategoryIds) <> '' THEN
SET
  v_json = CAST(
    CONCAT (
      '["',
      REPLACE (TRIM(p_CategoryIds), ',', '","'),
      '"]'
    ) AS JSON
  );
...
SELECT
  latitude,
  longitude INTO v_latitude,
  v_longitude
FROM
  countries
WHERE
  id = p_CenterCountryId;

END IF;

SET
  v_offset = (v_Page - 1) * v_PageSize;
...
WHERE
  ...
  AND (
    p_NotShownListing IS NULL
    OR l.id <> p_NotShownListing
  )
  AND (
    p_UserId IS NULL
    OR l.user_id = p_UserId
  )
  AND (
    p_SearchTerm IS NULL
    OR l.title LIKE CONCAT ('%', p_SearchTerm, '%')
  )
  AND (
    p_CreatedFrom IS NULL
    OR l.created_at >= p_CreatedFrom
  )
  AND (
    p_CreatedTo IS NULL
    OR l.created_at <= p_CreatedTo
  )
  ...
  AND (
    ...
    OR EXISTS (
      ...
      FROM
        JSON_TABLE (
          v_json,
          '$[*]' COLUMNS (cat_id VARCHAR(20) PATH '$')
        ) jt
        JOIN listings_categories lc ON lc.product_id = l.id
  ...
  AND (
    p_CountryId IS NULL
    OR u.country_id = p_CountryId
  )
  AND (
    p_MaxDistanceKm IS NULL
    OR p_CenterCountryId IS NULL
    OR (
      cu.latitude IS NOT NULL
      AND cu.longitude IS NOT NULL
      AND 111.045 * DEGREES (
        ACOS(
          COS(RADIANS (v_latitude)) * COS(RADIANS (cu.latitude)) * COS(RADIANS (cu.longitude - v_longitude)) + SIN(RADIANS (v_latitude)) * SIN(RADIANS (cu.latitude))
        )
      ) <= p_MaxDistanceKm
    )
  )
  /* --------- filtros directos ---------- */
  AND (
    p_ConditionFilter IS NULL
    OR l.`condition` = p_ConditionFilter
  )
  AND (
    p_BrandFilter IS NULL
    OR NULLIF(TRIM(p_BrandFilter), '') IS NULL
    OR l.brand LIKE CONCAT ('%', TRIM(p_BrandFilter), '%')
  )
  AND (
    p_TypeFilter IS NULL
    OR l.type LIKE CONCAT ('%', p_TypeFilter, '%')
  )
  /* --------- medios de pago ------------ */
  AND (
    COALESCE(p_AcceptsBarter, 0) + COALESCE(p_AcceptsCash, 0) + COALESCE(p_AcceptsTransfer, 0) + COALESCE(p_AcceptsCard, 0) = 0
    OR (
      (
        COALESCE(p_AcceptsBarter, 0) = 1
        AND l.accepts_barter = 1
      )
      OR (
        COALESCE(p_AcceptsCash, 0) = 1
        AND l.accepts_cash = 1
      )
      OR (
        COALESCE(p_AcceptsTransfer, 0) = 1
        AND l.accepts_transfer = 1
      )
      OR (
        COALESCE(p_AcceptsCard, 0) = 1
        AND l.accepts_card = 1
      )
    )
  )
  /* --------- precio -------------------- */
  AND (
    p_MinPrice IS NULL
    OR l.price >= p_MinPrice
  )
  AND (
    p_MaxPrice IS NULL
    OR l.price <= p_MaxPrice
  )
  /* --------- excluir usuario ----------- */
  AND (
    p_NotShownUser IS NULL
    OR l.user_id <> p_NotShownUser
  )
ORDER BY
  l.created_at ASC
LIMIT
